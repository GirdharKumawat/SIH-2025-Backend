...
# Initialize the router for HQ endpoints
hq_router = APIRouter()
...
async def set_user_verified(id: str):
    """
    Endpoint to set a user as verified by their ID
    """
...
async def add_members_to_group(group_id: str, add_members_data: AddMembersModel):
    ...
    try:
        ...
        return {"message": f"Members added to group {group_id}", "added_members": add_members_data.members}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")
...
async def delete_member_from_group(group_id: str, member_id: str):
    ...
    try:
        group = groups_collection.update_one(
            {"_id": ObjectId(group_id)},
